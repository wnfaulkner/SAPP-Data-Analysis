...
### PRODUCING DATA, TABLES, & CHARTS ###
  ...
  #for(i in l: length(school.names)){   #START OF LOOP BY SCHOOL
    ...
    #S3 Table for slide 3 "Overall Scale Performance"
      ...
      names(s3.outputs.df) <- c("category","school_avg","state_avg")
      s3.outputs.df$category <- s3.outputs.df$category %>% as.character
      s3.outputs.df$school_avg <- s3.outputs.df$school_avg %>% as.character %>% as.numeric %>% round(., digits = 2)
      s3.outputs.df$state_avg <- s3.outputs.df$state_avg %>% as.character %>% as.numeric %>% round(., digits = 2)

...
### LOAD & CLEAN DATA ###
{
  ...
  #IMPORT SAPP DATA AND STACK INTO SINGLE DATA FRAME
    ...
    #Eliminate responses before June 01, 2017
      sapp.df <- sapp.df[as.Date(sapp.df$created_datetime) > as.Date("2017-05-31"),]

    #Rename columns to include sapp profile name abbrevation
      sapp.df <-  cbind(
...
for(m in 1:length(reporting.districts)){ # START OF LOOP BY DISTRICT
  ...
  #1  ### SLIDE ## COVER SLIDE
  {
    ...
    title.m <- pot(
      "Self Assessment Practice Profile: Baseline",
      title.format
    )
    ...
    report.date <- pot(
      current.survey.file %>% file.mtime() %>% format(., c("%b, %d, %Y"))
    )
    ...
    pptx.m <- addParagraph(pptx.m, #Report Data Date
                           report.date, 
                           height = 0.67,
                           width = 7.76,
                           offx = 1.27,
  ...
  {  
    ...
    if(dim(users.df.m)[1] > 0){
      ...
      slide.graph <- ggplot(
        ...
        #Rectangles behind data labels above columns (to make readable even when intersected by average lines)
        geom_rect(
          aes(
            xmin = slide.data.df$school %>% as.factor %>% as.numeric-0.1,
            xmax = slide.data.df$school %>% as.factor %>% as.numeric+0.1,
            ymin = rev(slide.data.df$n) + max(slide.data.df$n)/25 - max(slide.data.df$n)/30,
            ymax = rev(slide.data.df$n) + max(slide.data.df$n)/25 + max(slide.data.df$n)/30,
          ), 
          fill = "white",
          alpha = 0.8
        ) +
    ...
    if(dim(users.df.m)[1] == 0){next()}else{}
  }  
  ...
  for(n in 1: length(sapp.profile.names.v)){  
    ...
    if(noviz.slide == TRUE){  
      ...
      slide.graph <- ggplot(
        ...
        geom_rect(
          aes(
            xmin = slide.data.df$school %>% as.factor %>% as.numeric-length(slide.data.df$school)/40,
            xmax = slide.data.df$school %>% as.factor %>% as.numeric+length(slide.data.df$school)/40,
            ymin = rev(slide.data.df$avg.proficiency) + max(slide.data.df$avg.proficiency)/25 - max(slide.data.df$avg.proficiency)/25 + 5,
            ymax = rev(slide.data.df$avg.proficiency) + max(slide.data.df$avg.proficiency)/25 + max(slide.data.df$avg.proficiency)/25 + 5,
          ), 
          fill = "white",
          alpha = 0.9
        ) +
        
        #Data labels above columns
        geom_text(
          aes(                                                         
            y = avg.proficiency + max(avg.proficiency)/25 + 5, 
